...
export default function AdminDashboard() {
  ...
  const handleDeleteUser = async (id: string, name: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setMessage({ text: data.error || 'Gagal menghapus akun', type: 'error' });
      }
    } catch (error) {
      setMessage({ text: 'Terjadi kesalahan sistem', type: 'error' });
    } finally {
  ...
  const handleClearAttendance = async (name: string) => {
    if (!confirm(`Apakah Anda yakin ingin menghapus SEMUA riwayat absen untuk ${name}?`)) return;

    setLoading(true);
    try {
      const res = await fetch(`/api/admin/attendance/${encodeURIComponent(name)}`, { method: 'DELETE' });
      const data = await res.json();
      if (data.success) {
        setMessage({ text: `Riwayat absen ${name} berhasil dihapus.`, type: 'success' });
      } else {
        setMessage({ text: data.error || 'Gagal menghapus riwayat absen', type: 'error' });
      }
    } catch (error) {
      setMessage({ text: 'Terjadi kesalahan sistem', type: 'error' });
    } finally {
      ...
      <main className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
          <div>
            ...
            <p className="text-gray-500">Kelola akun karyawan dan sistem absensi</p>
          </div>
          <div className="flex gap-3">
            <button onClick={() => router.push('/')} className="px-4 py-2 bg-blue-50 text-blue-600 font-semibold rounded-xl hover:bg-blue-100 transition-colors">
              Lihat Absensi
            </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 h-fit">
              <h2 className="text-lg font-bold mb-4 text-gray-800">Buat Akun Baru</h2>
              
              <form onSubmit={handleCreateUser} className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold mb-1 text-gray-700">Username Login</label>
                  <input
                    type="text"
                    name="username"
            ...
            {/* Pengaturan Sistem */}
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 h-fit">
              <h2 className="text-lg font-bold mb-4 text-gray-800">Pengaturan Sistem</h2>
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-semibold text-gray-800 text-sm">Batasi Absen Harian</p>
                  <p className="text-xs text-gray-500 mt-1">Mencegah user absen berulang kali di hari yang sama.</p>
                </div>
                <button
                ...
                >
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${enableDailyLimit ? 'translate-x-6' : 'translate-x-1'}`} />
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
            <h2 className="text-lg font-bold mb-4 text-gray-800">Daftar Pengguna Sistem</h2>
                    ...
                    users.map((u) => (
                      <tr key={u._id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="py-3 px-2 text-right space-x-2">
                          <button onClick={() => handleClearAttendance(u.name)} className="text-orange-600 bg-orange-50 px-2 py-1 rounded text-xs font-semibold hover:bg-orange-100 transition-colors">Hapus Absen</button>
                          <button onClick={() => handleDeleteUser(u._id, u.name)} className="text-red-600 bg-red-50 px-2 py-1 rounded text-xs font-semibold hover:bg-red-100 transition-colors">Hapus Akun</button>
                        </td>
                      </tr>
                    ))
                  )}
